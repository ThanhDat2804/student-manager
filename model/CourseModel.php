...
{
    $sqlInsert = "INSERT INTO `courses`( `course_name`, `slug`, `department_id`, `status`, `created_at`) 
    VALUES (:courseName, :slug, :departmentId, :statusCourse, :createdAt)";
    $checkInsert = false;
    $db = connectionDb();
...
function updateCourseById($name, $slug, $department_id, $status, $id)
{
    ...
    $sqlUpdate = "UPDATE `courses` SET `course_name`= :courseName,`slug`=:slug,`department_id`=:departmentId,`status`=:statusCourse,`updated_at`= :updatedAt 
